...
static bool inside_bb(const rr_node_property_t &node, const box &bb)
{
	...
	if (node.type == CHANX || node.type == CHANY) {
		...
		//if (node.xhigh >= bb.xmin && node.xlow <= bb.xmax
				//&& node.yhigh >= bb.ymin && node.ylow <= bb.ymax) {
			//inside = true;
		//}
		/* fully inside */
...
struct overlap_t {
	...
	vector<vector<int>> num_committed_mods; /* [tid][other_tid] */
};
...
class SpeculativeDeterministicRouter {
	...
	private:
		...
		pair<float, float> get_edge_weight(const RREdge &e)
		{
			...
			for (const auto &o : *_current_overlaps) {
				if (o == nullptr) {
					continue;
				}
				...
				if (!_acquired_overlaps[o->id]) {
					overlap_get_mods(*o, _instance, [this] (const mod_t &m) -> void {
	...
	while ((inet = router->sync.net_index++) < router->nets.size()) {
		...
		if (router->iter > 0) {
			...
			for (const auto &rr_node : router->state.back_added_rr_nodes[net.local_id]) {
				...
				update_first_order_congestion(router->state.congestion[net_router.get_instance()], rr_node, -1, rr_node_p.capacity, router->pres_fac);
				commit(router->overlaps[inet], router->g, rr_node, 1, net_router.get_instance());
			}
		}
...
#else
	router->sync.barrier->wait(
			[&router, &net_router] () -> void {
				++(*router->e_state[net_router.get_instance()].logical_clock);
			});
#endif
...
void init_net_overlaps(const vector<net_t *> &nets, exec_state_t *e_state, int num_threads, vector<vector<overlap_t *>> &overlaps)
{
	...
	for (int i = 0; i < nets.size(); ++i) {
		...
		for (int j = -window_size; j <= window_size; ++j) {
			...
			overlap_t *overlap;
			if (bg::intersection(cur_box, other_box, o)) {
			...
			overlaps[i].emplace_back(overlap);
		}
...
bool speculative_deterministic_route_hb(t_router_opts *opts)
{
	...
	init_net_overlaps(router.nets, router.e_state.data(), opts->num_threads, router.overlaps);
				...
				if (router.overlaps[i][j]) {
					for (int k = 0; k < opts->num_threads; ++k) {
						for (int l = 0; l < opts->num_threads; ++l) {
							...
							assert(router.overlaps[i][j]->num_committed_mods[k][l] == router.overlaps[i][j]->mods[l].size());
						}
					}
				}
			}
