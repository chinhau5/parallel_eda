...
bool mpi_route_load_balanced_nonblocking(t_router_opts *opts, struct s_det_routing_arch det_routing_arch, t_direct_inf *directs, int num_directs, t_segment_inf *segment_inf, t_timing_inf timing_inf)
{
	...
	mpi.buffer_size = sizeof(node_update_t)/sizeof(int) * num_vertices(partitioner.orig_g) * 0.1; //30% of total number of vertices
    ...
    for (iter = 0; iter < opts->max_router_iterations && !routed && !idling; ++iter) {
		...
		for (auto &net : partition_nets[mpi.rank]) {
			...
			if (iter > 0) {
				...
				if (cost_update_q.size() == route_tree_size) {
					...
					broadcast_rip_up_no_progress_reuse(net->local_id, &mpi);
					...
					auto broadcast_start = clock::now();
					broadcast_pending_cost_updates_reduced_no_progress_reuse(removed_nodes, net->local_id, -1, &mpi);
					mpi_perf.total_broadcast_time += clock::now()-broadcast_start;
				} else {
				...
				sync_irecv(nets, congestion, partitioner.orig_g, params.pres_fac, net_route_trees, &mpi, &mpi_perf);
			...
			if (!sinks.empty()) {
				...
				route_net_mpi_send_recv_nonblocking(partitioner.orig_g, net->vpr_id, net->local_id, &net->source, sinks, params, nets, states, congestion, route_trees[net->local_id], net_timing[net->vpr_id], net_route_trees, routed_sinks[net->local_id], unroutable_sinks[net->local_id], &mpi, &perf, &mpi_perf, opts->sync_only_once, false);
		...
		while (!all_done(&mpi)) {
			sync_irecv(nets, congestion, partitioner.orig_g, params.pres_fac, net_route_trees, &mpi, nullptr);
			++num_last_syncs;
		}
		...
		for (int i = 0; i < mpi.pending_send_data_raw.size(); ++i) {
			assert(mpi.pending_send_data_raw[i].second == 0);
		}
