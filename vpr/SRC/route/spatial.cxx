...
void init_route_structs(const RRGraph &g, const vector<net_t> &nets, const vector<net_t> &global_nets, route_state_t **states, congestion_mpi_t **congestion, vector<route_tree_t> &route_trees, t_net_timing **net_timing)
{
	...
	*congestion = new congestion_mpi_t[num_vertices(g)];
    for (int i = 0; i < num_vertices(g); ++i) {
...
void init_route_structs_mpi(const RRGraph &g, const vector<net_t> &nets, const vector<net_t> &global_nets, route_state_t **states, congestion_mpi_t **congestion, MPI_Win *win, vector<route_tree_t> &route_trees, t_net_timing **net_timing)
{
	*states = new route_state_t[num_vertices(g)];
	for (int i = 0; i < num_vertices(g); ++i) {
		(*states)[i].rr_node = -1;
	...
	assert(MPI_Win_allocate(sizeof(congestion_mpi_t)*num_vertices(g), 1, MPI_INFO_NULL, MPI_COMM_WORLD, congestion, win) == MPI_SUCCESS);
    for (int i = 0; i < num_vertices(g); ++i) {
        (*congestion)[i].acc_cost = 1;
        (*congestion)[i].pres_cost = 1;
        (*congestion)[i].occ = 0;
...
void init_route_structs(const RRGraph &g, const vector<net_t> &nets, const vector<net_t> &global_nets, int num_threads, vector<route_state_t *> &states, congestion_t **congestion, vector<route_tree_t> &route_trees, t_net_timing **net_timing)
{
...
void sync_net_delay(const vector<pair<box, net_t *>> &nets_to_route, int procid, int num_procs, int initial_num_procs, int current_level, MPI_Comm comm, t_net_timing *net_timing)
{
...
void recv_route_tree(net_t *net, const RRGraph &g, vector<vector<sink_t *>> &routed_sinks, route_state_t *states, congestion_mpi_t *congestion, float pres_fac, vector<route_tree_t> &route_trees, t_net_timing *net_timing, int from_procid, MPI_Comm comm)
{
	MPI_Status status;
	...
	for (int i = 0; i < count; ++i) {
		...
		route_tree_set_node_properties(get_vertex_props(route_trees[net->local_id].graph, rt_node), rr_node_p.type != IPIN && rr_node_p.type != SINK, rr_edge_to_parent, recv[i].upstream_R, recv[i].delay);
...
bool mpi_spatial_route_new(t_router_opts *opts, struct s_det_routing_arch det_routing_arch, t_direct_inf *directs, int num_directs, t_segment_inf *segment_inf, t_timing_inf timing_inf)
{
...
bool mpi_spatial_route(t_router_opts *opts, struct s_det_routing_arch det_routing_arch, t_direct_inf *directs, int num_directs, t_segment_inf *segment_inf, t_timing_inf timing_inf)
{
    ...
    for (iter = 0; iter < opts->max_router_iterations && !routed; ++iter) {
		...
		while (i < nets_to_route.size()) {
			...
			//}
			route_tree_rip_up_marked_mpi(route_trees[net->local_id], partitioner.orig_g, partitioner.result_pid_by_level[current_level], procid, congestion, win, params.pres_fac);
