...
static void add_route_tree_to_heap(
	...
	if (rt_node->re_expand) {
		...
		zlog_debug(route_inner_log, "Not expanding %d\n", rt_node->inode);
	}
	...
	public:
		...
		void operator()(argument_type item, tbb::parallel_do_feeder<argument_type> &feeder) const
		{
			...
			while (!found_sink && inode != target_node) {
				...
				zlog_debug(route_inner_log, "Current: %s old_t: %g new_t: %g old_b: %g new_b: %g old_prev_node: %d new_prev_node: %d in_route_tree: %d added_from_route_tree %d\n", buffer, old_tcost, new_tcost, old_back_cost, new_back_cost, l_rr_node_route_inf[inode].prev_node, current_heap_item.u.prev_node, in_route_tree ? 1 : 0, added_from_route_tree ? 1 : 0);
				...
				if (in_route_tree && added_from_route_tree) {
					/*assert(l_rr_node_route_inf[inode].prev_node == current_heap_item.u.prev_node);*/
					/*assert(l_rr_node_route_inf[inode].prev_edge == current_heap_item.prev_edge);*/
					/*assert(l_rr_node_route_inf[inode].path_cost == current_heap_item.cost);*/ /* astar cost changes, so we can't check for equality here */
					/*assert(l_rr_node_route_inf[inode].backward_path_cost == current_heap_item.backward_path_cost);*/ /* when after adding to heap, the backward path cost does not include the congestion cost */
				...
				while (num_spawned_workers < num_threads && !local_heap.empty()) {
					...
					feeder.add({ current_heap_item, 0 });
					++num_spawned_workers;
				}
				spawn_lock.unlock();
			}
...
void prime(int inode)
{
	/*int required = 8;*/
	/*vector<int> seed_nodes;*/
	/*while (seed_nodes.size() < required) {*/
			...
			/*float new_tcost = current.cost;*/
			/*float new_back_cost = current.backward_path_cost;*/
			...
			/*heap_index_lock.lock();*/
			/*int local_heap_index = heap_index;*/
			/*heap_index = (heap_index+1) % num_threads;*/
			/*heap_index_lock.unlock();*/
...
static boolean tbb_parallel_timing_driven_route_net_2(
		int inet,
...
/*			free_route_tree(rt_root);*/
		...
		while (!found_sink && !temp_heap.empty()) {
			vector<RouteWorker2::argument_type> items;
			int num_spawned_workers = 0;
			int heap_index = 0;
			while (!temp_heap.empty() && num_spawned_workers < num_threads) {
				items.push_back({ temp_heap.top(), 0 });
				temp_heap.pop();
				++num_spawned_workers;
			}

			RouteWorker2 worker(l_rr_node_route_inf_lock, found_sink, current, num_found_sink, spawn_lock,
					heap_index, num_spawned_workers, modified_inodes, modified_inodes_lock);
			/*worker.heaps = heaps;*/
...
class RouteWorker {
	public:
		...
		RouteWorker(int &heap_index, std::mutex &heap_index_lock, std::priority_queue<struct s_heap> *heaps, std::mutex &l_rr_node_route_inf_lock,
				bool &found_sink, struct s_heap &sink_node, std::mutex &sink_lock)
			: heap_index(heap_index), heap_index_lock(heap_index_lock), heaps(heaps), l_rr_node_route_inf_lock(l_rr_node_route_inf_lock),
			found_sink(found_sink), sink_node(sink_node), sink_lock(sink_lock)
		{
		}

		typedef struct s_heap argument_type;
		void operator()(struct s_heap current, tbb::parallel_do_feeder<struct s_heap> &feeder) const
		{
			sink_lock.lock();
			if (found_sink) {
				sink_lock.unlock();
			...
			for (int i = 0; i < branch_factor && !local_heap.empty(); ++i) {
				current = local_heap.top();
				local_heap.pop();
				feeder.add(current);
			}
...
/*	largest_criticality = pin_criticality[sink_order[1]];*/
	...
	for (itarget = 1; itarget <= num_sinks; itarget++) {
		...
		zlog_debug(route_inner_log, "Adding route tree to heap\n");
		add_route_tree_to_heap(rt_root, target_node, target_criticality,
				opts->astar_fac, l_rr_node_route_inf, inet, heaps[0], net_route->num_heap_pushes);

		if (heaps[0].empty()) { /* Infeasible routing.  No possible path for net. */
			zlog_fatal(route_inner_log, "Cannot route net #%d (%s) to sink #%d -- no possible path.\n",
					   inet, clb_net[inet].name, itarget);
			for (const auto &modified : modified_inodes) {
				l_rr_node_route_inf[modified].path_cost = HUGE_POSITIVE_FLOAT;
...
/*			free_route_tree(rt_root);*/
		...
		vector<struct s_heap> items;
		int i = 0;
		const int branch_factor = 8;
		while (!heaps[0].empty() && i < branch_factor) {
			items.push_back(heaps[0].top());
			heaps[0].pop();
			++i;
		}
		...
		RouteWorker worker(heap_index, heap_index_lock, heaps, l_rr_node_route_inf_lock, found_sink, current, sink_lock);
		worker.l_rr_node_route_inf = l_rr_node_route_inf;
		worker.l_rr_node_to_rt_node = l_rr_node_to_rt_node;
		worker.target_node = target_node;
		worker.inet = inet;
...
static boolean parallel_timing_driven_route_net(
		int inet,
		...
		timeval *time)
{
	...
	/*tbb::parallel_for(tbb::blocked_range<int>(0, 10), [](const tbb::blocked_range<int> &range) -> void*/
			/*{*/
			/*}*/
			/*);*/
...
/*	largest_criticality = pin_criticality[sink_order[1]];*/
	...
	for (itarget = 1; itarget <= num_sinks; itarget++) {
		...
		t_rt_node *rt_node = rt_root;
		assert(rt_node->re_expand);
		t_linked_rt_edge *linked_rt_edge = rt_node->u.child_list;
		while (linked_rt_edge != NULL) {
			t_rt_node *child_node = linked_rt_edge->child;
			assert(child_node->re_expand);
			linked_rt_edge = linked_rt_edge->next;
		}
		...
		zlog_debug(route_inner_log, "Adding route tree to heap\n");
		add_route_tree_to_heap(rt_root, target_node, target_criticality,
				opts->astar_fac, l_rr_node_route_inf, inet, heap, net_route->num_heap_pushes);
		...
		while (inode != target_node) {
			...
			if (old_tcost > new_tcost && old_back_cost > new_back_cost) {
				...
				if (in_route_tree && not_in_previous_route_tree && current.u.prev_node != l_rr_node_route_inf[inode].prev_node) {
					...
					net_route->num_heap_pushes += timing_driven_expand_neighbours(
							&current, target_node,
							inet,
							opts->bend_cost, opts->astar_fac, target_criticality, highfanout_rlim, false,
							l_rr_node_route_inf,
							heap);
				}
			}
...
static void *worker_thread(void *arg)
{
	...
	while (true) {
		...
		while (inet >= 0 && !routed_last) {
			...
			is_routable = parallel_timing_driven_route_net(
					inet,
	...
	for (itry = 1; itry <= router_opts.max_router_iterations; itry++) {
		...
		for (i = 0; i < num_nets; i++) {
			...
			if (clb_net[inet].is_global == FALSE) { /* Skip global nets. */
				...
				is_routable = tbb_parallel_timing_driven_route_net_2(
						inet,
...
static boolean try_fine_grained_parallel_timing_driven_route_old(struct s_router_opts router_opts,
		t_net_timing *net_timing, t_ivec ** clb_opins_used_locally, boolean timing_analysis_enabled, int round, std::mt19937 &mt) {
...
/*						rt_node_of_sink);*/
		...
		for (auto iter = sorted_num_heap_pushes.rbegin(); iter != sorted_num_heap_pushes.rend(); ++iter) {
			...
			zlog_debug(route_outer_log, "Net: %d BB Area: %d Num sinks: %d Num pushes: %d Num pops: %d Pop/Push: %g Route time: %g\n", inet, area, clb_net[inet].num_sinks, iter->first, tinfo[0].net_route[inet].num_heap_pops, (double)tinfo[0].net_route[inet].num_heap_pops*100/iter->first, elapsed);
			fprintf(heap_pushes, "%d %d %d\n", area, clb_net[inet].num_sinks, iter->first);
		}
		fclose(heap_pushes);
		...
		t_net_timing *net_timing, t_ivec ** clb_opins_used_locally, boolean timing_analysis_enabled)
{
	...
	for (int i = 0; i < num_repetitions; ++i) {
		try_fine_grained_parallel_timing_driven_route_old(router_opts, net_timing, clb_opins_used_locally, timing_analysis_enabled, i, mt);
	}
}
